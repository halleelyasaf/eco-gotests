...
var (
	...
	storageClassName = tncparams.StorageClassName
	volumeMode       = corev1.PersistentVolumeBlock
)

// VerifyODFNamespaceExists asserts namespace for ODF exists.
func VerifyODFNamespaceExists(ctx SpecContext) {
	err := apiobjectshelper.VerifyNamespaceExists(APIClient, tncparams.ODFNamespace, time.Second)
	Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("Failed to pull %q namespace", tncparams.ODFNamespace))
} // func VerifyODFNamespaceExists (ctx SpecContext)
...
func VerifyODFDeployment(ctx SpecContext) {
	for _, operatorPod := range odfOperatorDeployments {
		glog.V(tncparams.TncLogLevel).Infof("Confirm that odf %s pod was deployed and running in %s namespace",
			operatorPod, tncparams.ODFNamespace)

		odfPods, err := pod.ListByNamePattern(APIClient, operatorPod, tncparams.ODFNamespace)
		Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("No %s pods were found in %s namespace; %v",
			operatorPod, tncparams.ODFNamespace, err))
		...
		if err != nil {
			...
			glog.Fatalf("%s pod in %s namespace in a bad state: %s",
				odfPodName, tncparams.ODFNamespace, odfPodLog)
		}
	}

	for _, operatorDeployment := range odfOperatorDeployments {
		glog.V(tncparams.TncLogLevel).Infof("Confirm that %s deployment is running in %s namespace",
			operatorDeployment, tncparams.ODFNamespace)

		odfDeployment, err := deployment.Pull(APIClient, operatorDeployment, tncparams.ODFNamespace)
		Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("%s deployment not found in %s namespace; %v",
			operatorDeployment, tncparams.ODFNamespace, err))
...
func VerifyODFConfig(ctx SpecContext) {
	glog.V(tncparams.TncLogLevel).Infof("Enable odf-console")
	...
	glog.V(tncparams.TncLogLevel).Infof("Start to configure ODF StorageCluster")
	...
	storageclusterObj, err := storage.NewStorageClusterBuilder(APIClient, storageclusterName, tncparams.ODFNamespace).
	...
	Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("Failed to create storageCluster %s instance in %s namespace; "+
		"%v", storageclusterName, tncparams.ODFNamespace, err))
...
func VerifyODFSuite() {
	Describe(
		"ODF validation",
		Label(tncparams.LabelTncOperators), func() {
			It(fmt.Sprintf("Verifies %s namespace exists", tncparams.ODFNamespace),
